...
| `-ProxyUser` | Логин SOCKS5. Пароль в файл **не пишется** — введете в GUI один раз. | `relayuser` |
...
| `FOREIGN_SSH_USER` | Пользователь на VPS, которому даем доступ по ключу. | `root` |
...
| Переменная | Назначение | По умолчанию |
|---|---|---|
| `RU_SERVER_IP` | **Обязательно.** Публичный IPv4 РФ-сервера (бастиона) — единственный источник, которому будет разрешен SSH к VPS. | — |
...
**Что выдает:** готовые ссылки `tg://proxy?...` и `https://t.me/proxy?...` — открой на телефоне/в Telegram Desktop, подтверди подключение.
